...
while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    if [[ -d "$target/.git" ]]; then
        log "Updating $name"
        git -C "$target" pull --ff-only || warn "pull failed for $name"
    else
        log "Cloning $name"
        git clone --depth=1 "$url" "$target" || { warn "clone failed for $url"; continue; }
    fi

    if [[ -f "$target/requirements.txt" ]]; then
        log "Installing Python deps for $name"
        "$PIP" install -r "$target/requirements.txt" || warn "pip install failed for $name"
    fi
done < "$CONFIG"
